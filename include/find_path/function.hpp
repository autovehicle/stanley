...
double getYawErr (const vector<egoPath>& path) {
    ...
    if (next_idx >= path.size()) {
        next_idx = path.size() - 1; // 마지막 인덱스
        closest_idx = next_idx - 2; // 그 앞 인덱스 (이래야 de, dn이 0이 안 됨)
    }
    ...
    if(yaw_err > M_PI) yaw_err = yaw_err - 2*M_PI;
    else if (yaw_err < -M_PI) yaw_err = yaw_err + 2*M_PI;
...
int findConerIdx (const vector<egoPath>& path) {
    static int coner_index = 0;
    int closest_index = findClosestPoint(path) + 30;

    int next_index = closest_index + 31;
    int next_index2 = closest_index + 32;
    
    double de = path[next_index].e - path[closest_index].e;
    double dn = path[next_index].n - path[closest_index].n;

    double path_yaw1 = atan2(dn, de);
    
    double de2 = path[next_index2].e - path[next_index].e;
    double dn2 = path[next_index2].n - path[next_index].n;

    double path_yaw2 = atan2(dn2, de2);

    double curvance = 1000*(path_yaw2 - path_yaw1);

    if (curvance < -50.0 || curvance > 50.0) {
        coner_index = closest_index;
    }
