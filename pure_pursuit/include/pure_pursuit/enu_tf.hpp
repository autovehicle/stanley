...
void gpsTf (const morai_msgs::GPSMessage::ConstPtr& msg, double& current_e, double& current_n) {
    ...
    // 파일의 절대경로 찾기
    std::string package_path = ros::package::getPath("pure_pursuit");
...
bool loadPath() {
    ...
    // ros가 pure_pursuit 패키지의 절대경로를 찾음
    string pkg_path = ros::package::getPath("pure_pursuit");
...
double getSignedDistanceErr(const vector<egoPath>& path, int closest_idx) {
    ...
    // 1. 경로 세그먼트(직선)의 시작점(p_start)과 끝점(p_end)을 정의
    ...
    if (closest_idx >= path.size() - 1) { // 경로 끝에 도달
        ...
        p_start = path[closest_idx];
        p_end = path[closest_idx + 1];
    }
    ...
    double vecB_e = current_e - p_start.e;
    double vecB_n = current_n - p_start.n;

    // 4. 2D 외적 (A.x*B.y - A.y*B.x) 계산
    //    (A.e * B.n) - (A.n * B.e)
    ...
    // 6. 오차의 크기 계산 (기존 getDistanceErr 방식)
    //    가장 가까운 *점* (path[closest_idx])까지의 절대 거리
